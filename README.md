...
| **追蹤停損** | 獲利超過 1.5R 後，停損跟著最高點走，距離 0.8R | 讓剩下 2/3 的部位有機會跑遠，同時鎖住已有的利潤 |
...
- **出場套用本文開頭「部位管理」那段 A/B 實測驗證過的同一組規則**
  ...
  每次執行時檢查獲利有沒有過門檻——到 +0.5R 搬到成本價 +0.05R，超過 +1.5R
  之後改成追蹤停損、距離最高獲利 0.8R，只會愈移愈緊，不會反向鬆開
...
| 變數 | 預設 | 意思 |
|---|---|---|
| `AUTO_TRADE_RISK_PCT` | 2 | 每筆風險占 Demo 帳戶可用餘額的 %（調高會讓單筆賺賠都放大，同時能同時撐住的倉位數會變少）。模擬盤紀錄出現過連 19 敗：5% 會虧掉約 62%，2% 約 32% |
...
| `AUTO_TRADE_MAX_OPEN_RISK_PCT` | 6 | 所有追蹤中部位「停損打到還會虧多少」加總的上限（占帳戶 %，0＝不限制）。幣圈同漲同跌，同時開一堆同方向的單等於同一個賭注。搬到成本價以上的部位剩餘風險是 0、不佔額度。超過上限的新訊號只通知不下單 |
| `AUTO_TRADE_MIN_STOP_PCT` | 1 | 停損距離（占進場價 %）低於這個值就只通知不下單（0＝不限制）。手續費按倉位價值收，停損 0.5% 時一進一出的吃單費約吃掉 0.22R；回測裡停損 <1% 的單扣完手續費是負的 |
...
- 模擬盤紀錄裡**符合目前自動下單規則**（多空都算、非 Order Block、≥ 65 分、停損 ≥ 1%）
...
- 前一個月 `AUTO_TRADE_RISK_PCT` 用 **1**（300U 每單最多虧 3U），確認實盤跟 Demo 表現一致再回到 2
- 小資金會有部分幣種下不了單：Bybit 每個合約有最小下單量，分批出場的每一段都要大於它，
  像 BTC 這種單價高的，300U 的倉位切成三段常常不夠，程式會直接跳過（Discord 會寫原因）
- 手續費已經由 `AUTO_TRADE_MIN_STOP_PCT` 擋掉最傷的那一類（停損太近的單）

**複利的現實（回測數字，實盤通常會打折）：**

回測在目前規則下扣手續費後大約每筆 +0.10R；保守抓一半 +0.05R。每單風險 2% 時：
