...
| 規則 | 做什麼 | 為什麼 |
|---|---|---|
| **保本鏢**（2026-09 起關閉） | 原本到 +0.5R 先出掉 1/3 | 2026-09 重測（兩組各 15 幣、前後半段）：不先出、整筆留著追目標，四格都持平或更好，所以預設關閉（`scalpR: 0`），停損移到成本價照舊 |
| **移到成本價** | 同時把停損拉到進場價 +0.05R | 0.05R 是用來蓋手續費的，不然「保本」實際上還是小虧 |
| **追蹤停損** | 獲利超過 1R 後，停損跟著最高點走，距離 0.5R | 讓部位有機會跑遠，同時鎖住已有的利潤 |
...
| `AUTO_TRADE_MIN_SIZE_PCT` | 50 | 倉位大小是用「可用餘額」算的，其他部位佔掉保證金後新單會變很小。這筆只能開到「用帳戶總額算出來該有大小」的這個 % 以下就不開、只通知（0＝不限制） |
